...
title: CODEX
type: codex-config
permalink: vllm-mlx-fork/codex
---

# CODEX.md — vllm-mlx-fork

> Project-specific context only. Global settings live in `~/.codex/CODEX.md`.
...
```bash
scripts/serve_profile.sh mllm-default mlx-community/MAI-UI-8B-6bit
```
...
- canonical roadmap and planning entity is `vllm-mlx-fork-codex`
...
- __Canonical entity ID__: `vllm-mlx-fork-codex`
- __Operator ID__: `operator-swaylen`
- __Memory writes__: use the canonical entity ID for shared project memory and state writes
- __Legacy aliases__: `vllm-mlx-fork-claude` and `vllm-mlx-fork-gemini` may still appear in older memo traffic, but are not the canonical roadmap entity IDs
