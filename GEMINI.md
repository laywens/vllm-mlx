...
title: GEMINI
type: gemini-config
permalink: vllm-mlx-fork/gemini
---

# GEMINI.md — vllm-mlx-fork

> Project-specific context only. Global settings live in `~/.gemini/GEMINI.md`.
...
- __Canonical entity ID__: `vllm-mlx-fork-gemini`
- __Operator ID__: `operator-swaylen`
- __Memory writes__: use the canonical entity ID for shared project memory and state writes
- __Legacy note__: older memo traffic may reference `vllm-mlx-fork-codex`; that entity is now succeeded by `vllm-mlx-fork-claude` as primary runtime
