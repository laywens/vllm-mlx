...
def _install_fake_qwen35_modules(monkeypatch):
    ...
    def fake_sdpa(queries: mx.array, keys: mx.array, values: mx.array, **kwargs):
        ...
        if mask is not None:
            call_log.append({"mask_len": int(mask.shape[-1])})
        return queries

    fake_qwen35_mod = types.ModuleType("mlx_vlm.models.qwen3_5.language")
    fake_qwen35_mod.Qwen3_5Attention = DummyAttention
    fake_qwen35_mod.apply_multimodal_rotary_pos_emb = (
        fake_apply_multimodal_rotary_pos_emb
    )

    fake_lm_base_mod = types.ModuleType("mlx_lm.models.base")
    fake_lm_base_mod.scaled_dot_product_attention = fake_sdpa
...
def test_qwen35_patch_uses_int_cache_offset_for_mask_slicing(monkeypatch):
    attention_cls, cache_cls, call_log = _install_fake_qwen35_modules(monkeypatch)

    assert patch_qwen35_attention_for_batching() is True

    attn = cast(Any, attention_cls())
    x = mx.zeros((1, 4, 1024), dtype=mx.float32)
    cache = cache_cls(offset=mx.array([3, 5]))
    mask = mx.zeros((1, 1, 4, 12), dtype=mx.float32)

    out = attn(x, mask=mask, cache=cache, position_ids=None)

    assert out.shape[1] == 4
    assert call_log[-2] == {"q_len": 4, "cos_len": 4}
    assert call_log[-1] == {"mask_len": 10}
